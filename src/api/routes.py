...
async def preview_prompt(query: str = "このプロジェクトの概要を説明してください"):
    ...
    result = gen.generate(state.index, query, state.summarized)

    return PreviewResponse(
        masked_prompt=result.context[:5000] + ("..." if len(result.context) > 5000 else ""),
        estimated_tokens=result.estimated_tokens,
        files_included=result.files_included,
        files_truncated=result.files_truncated,
        masking_log=[
            {"token": e.token, "pattern": e.pattern_name}
...
async def query(req: QueryRequest):
    ...
    prompt_result = gen.generate(state.index, req.query, state.summarized)
